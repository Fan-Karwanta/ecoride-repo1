...
export const signin = async (
  ...
  try {
    ...
    console.log("Using API URL:", `${BASE_URL}/auth/signin`);
    
    // Test if the server is reachable
    try {
      const testResponse = await axios.get(`${BASE_URL}/auth`);
...
export const updateUserProfile = async (profileData: {
  ...
  try {
    const token = await tokenStorage.getString("access_token");
    const res = await axios.put(`${BASE_URL}/auth/profile`, profileData, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
